...
led FWNO = led(Brain.ThreeWirePort.E);
inertial Inertial13 = inertial(PORT13);

led FWReady = led(Brain.ThreeWirePort.G);
...
void autonomous(void) {
  ...
  // backs up into roller a bit to gain traction 
  front_right.spinFor(reverse, .1, turns, false); 
  front_left.spinFor(forward, .1, turns, false); 
  back_right.spinFor(reverse, .1, turns, false); 
  back_left.spinFor(forward, .1, turns, false); 
  wait(1, seconds); 
  // spins roller 
  intakeroller.spinFor(reverse, .2, turns); 
  wait(1, seconds); 
  front_right.spinFor(forward, .3, turns, false); 
  front_left.spinFor(reverse, .3, turns, false); 
  back_right.spinFor(forward, .3, turns, false); 
  back_left.spinFor(reverse, .3, turns, false); 
  // turns to middle of field 
  front_right.spinFor(forward, .9, turns, false); 
  front_left.spinFor(forward, .9, turns, false); 
  back_right.spinFor(forward, .9, turns, false); 
  back_left.spinFor(forward, .9, turns, false); 
  wait(1, seconds); 
  // velocity changes 
  front_left.setVelocity(125, rpm); 
  front_right.setVelocity(125, rpm);
  back_right.setVelocity(125, rpm); 
  back_left.setVelocity(125, rpm); 
  // drives to middle of the field 
  Inertial13.setHeading(45, degrees); 
  targetspeed = 8; 
  task flyPIDTask = task(FlyPID); 
  front_right.spinFor(forward, 5.5, turns, false); 
  front_left.spinFor(reverse, 5.5, turns, false); 
  back_right.spinFor(forward, 5.5, turns, false); 
  back_left.spinFor(reverse, 5.5, turns, true); 
  wait(.5, seconds); 
  // velocity changes 
  front_left.setVelocity(100, rpm); 
  front_right.setVelocity(100, rpm); 
  back_right.setVelocity(100, rpm); 
  back_left.setVelocity(100, rpm); 
  // turn left to aim at goal 
  front_right.spinFor(reverse, 1.3, turns, false); 
  front_left.spinFor(reverse, 1.3, turns, false); 
  back_right.spinFor(reverse, 1.3, turns, false); 
  back_left.spinFor(reverse, 1.3, turns, true); 
  // scoot forward 
  front_right.spinFor(forward, .3, turns, false); 
  front_left.spinFor(reverse, .3, turns, false); 
  back_right.spinFor(forward, .3, turns, false); 
  back_left.spinFor(reverse, .3, turns, true); 
  // shoot at goal 
  wait(1, seconds); 
  indexer.spinFor(forward, 1, turns); 
  Controller1.rumble("-"); 
  wait(.2, seconds); 
  indexer.spinFor(forward, 1, turns); 
  Controller1.rumble("-"); 
